...
#@login_required(login_url=settings.BASE_URL + 'user/login/')
def logout(request):
    auth_logout(request)
    return HttpResponseRedirect(settings.BASE_URL)
...
def create_team(request, event_id = None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect('%suser/teams/%s/' % (settings.BASE_URL, team.id))
...
def join_team(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect('%smyshaastra/' % settings.BASE_URL)
...
def drop_out(request, team_id = None):
    ...
    if team is not None:
        ...
        if user == team.leader:
            ...
            return HttpResponseRedirect('%sevents/' % settings.BASE_URL)
    raise Http404
...
def dissolve_team(request, team_id = None):
    ...
    if team is not None:
        if team.members.all().count() > 1:
            ...
            team.delete()
            if hospi_override:
                return HttpResponseRedirect('%scontrolroom/home/' % settings.BASE_URL)
            return HttpResponseRedirect('%sevents/' % settings.BASE_URL)
    raise Http404
